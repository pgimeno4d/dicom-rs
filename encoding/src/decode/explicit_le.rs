...
mod tests {
    ...
    const RAW: &[u8] = &[
        ...
        0x10, 0x00, 0x00, 0x40,     // (0010,4000) (LE) Patient Comments
            ...
            0x04, 0x00,             // Length: 4 bytes (LE)
                // String "None"
                b'N', b'o', b'n', b'e',
        // FIXME: OB, OD, OF, OL, OV, OW not tested - add?
        ...
        0x18, 0x00, 0x16, 0x60,     // (0018,6016) (LE) Region Flags
            ...
            0x04, 0x00,             // Length: 4 bytes (LE)
                // Value: 1 (LE)
                0x01, 0x00, 0x00, 0x00,
        0x01, 0x00, 0x34, 0x12,     // (0001,1234) (LE) (Private data element)
    ...
    fn decode_data_elements() {
        ...
        fn test_vr(
            ...
            assert_eq!(elem.length(), Length(len as u32));
            let buffer = read_n(cursor, len);
        ...
        // read various VRs
        test_vr(&dec, &mut cursor, 0x0008, 0x0054, VR::AE, 6, b"TITLE ");
        test_vr(&dec, &mut cursor, 0x0010, 0x1010, VR::AS, 2, b"8Y");
        test_vr(
            &dec,
            &mut cursor,
            0x0072,
        ...
        test_vr(&dec, &mut cursor, 0x0010, 0x0040, VR::CS, 2, b"O ");
        test_vr(&dec, &mut cursor, 0x0010, 0x0030, VR::DA, 8, b"19800101");
        test_vr(&dec, &mut cursor, 0x0010, 0x1020, VR::DS, 4, b"1.70");
        test_vr(
            &dec,
            &mut cursor,
            0x0008,
        ...
        test_vr(&dec, &mut cursor, 0x0020, 0x0013, VR::IS, 8, b"1234567 ");
        test_vr(&dec, &mut cursor, 0x0010, 0x0020, VR::LO, 10, b"P12345678X");
        test_vr(&dec, &mut cursor, 0x0010, 0x4000, VR::LT, 4, b"None");
        test_vr(&dec, &mut cursor, 0x0010, 0x0010, VR::PN, 8, b"Doe^John");
        test_vr(&dec, &mut cursor, 0x0040, 0x9210, VR::SH, 4, b"LBL ");
        test_vr(
            &dec,
            &mut cursor,
            0x0018,
        ...
        test_vr(&dec, &mut cursor, 0x0040, 0x0280, VR::ST, 10, b"No comment");
        test_vr(
            &dec,
            &mut cursor,
            0x0072,
        ...
        test_vr(&dec, &mut cursor, 0x0010, 0x0032, VR::TM, 6, b"123456");
        test_vr(&dec, &mut cursor, 0x0008, 0x0119, VR::UC, 4, b"Code");
        test_vr(
            &dec,
            &mut cursor,
            0x0018,
        ...
        test_vr(&dec, &mut cursor, 0x0008, 0x0040, VR::US, 2, &[0x07, 0x87]);
        test_vr(&dec, &mut cursor, 0x0018, 0x9917, VR::UT, 8, b"No text ");
        test_vr(
            &dec,
            &mut cursor,
            0x0008,
